...
// first, set up the translation
    ...
    for (i = 0; i < numPages; i++){
		...
		if(pageTable[i].physicalPage==-1){
			int j=0;
			for(;j<i;j++) memoryManager->FreePage(pageTable[j].physicalPage);
	...
	unsigned int codeSize = divRoundUp(noffH.code.size, PageSize);
	unsigned int dataSize = divRoundUp(noffH.initData.size, PageSize);
	...
	if (noffH.code.size > 0 || noffH.initData.size > 0){
		...
		for (unsigned int x=codeSize; x < dataSize; x++)
			executable -> ReadAt(&(machine -> mainMemory[pageTable[x].physicalPage * PageSize]),
				PageSize, noffH.initData.inFileAddr + (x-codeSize) * PageSize);
	}
